				...
				<div class="element-box-content">
					...
					<form class="mt-4" id="dform" method="post">
						<div class="form-group row">
							<label class="col-sm-2 col-form-label">Kategori SK</label>
							<label class="col-sm-8 col-form-label"><?=$arrKatSK[$kat_sk]?></label>
						</div>
...
$(document).ready(function(){
	...
	$.each(JSON.parse(json), function (index, item) {
        html += nestableTree(item,true,'<?=BE_MAIN_HOST?>/sdm/unit-kerja/update?origin=so&id=','<?=BE_MAIN_HOST?>/sdm/unit-kerja/update?origin=so&do=update_status_read&stt=1&id=');
    });
