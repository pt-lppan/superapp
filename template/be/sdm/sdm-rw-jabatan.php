...
<script>
	...
	function setupDetail(no_urut, kat, id, no_sk, tglsk, tglmulai, tglselesai, label_jabatan, id_jabatan, berkas, label_jabatan2, plt, kontrak, pencapaian, isDelEnabled) {
		...
		$('#tglmulai<?= $acak ?>' + kat + '_' + no_urut).datepick({
			monthsToShow: 1,
			dateFormat: 'yyyy-mm-dd'
		});
		$('#tglselesai<?= $acak ?>' + kat + '_' + no_urut).datepick({
			monthsToShow: 1,
			dateFormat: 'yyyy-mm-dd'
		});
		...
		$(document).on('focus', '#nama_jabatan<?= $acak ?>' + kat + '_' + no_urut + '', function(e) {
			$(this).autocomplete({
				...
				change: function(event, ui) {
					if ($(this).val().length == 0) $('#id_jabatan<?= $acak ?>' + kat + '_' + no_urut + '').val('');
				},
				select: function(event, ui) {
					$('#id_jabatan<?= $acak ?>' + kat + '_' + no_urut + '').val(ui.item.id);
				}
			});
		});
