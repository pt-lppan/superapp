<ul class="breadcrumb">
	...
	<li class="breadcrumb-item">
		<span>Daftar WO Pengembangan</span>
	</li>
</ul>
		...
		<div class="element-wrapper">
			<div class="clearfix">	
				<nav class="element-actions">
					<a class="btn btn-primary" href="<?=BE_MAIN_HOST?>/manpro/proyek/wo-pengembangan-update">Tambah Work Order</a>
				</nav>
					...
					<table class="table table-bordered table-hover table-sm">
						<thead class="thead-light">
							<tr>
								...
								<th><b>Pembuat&nbsp;WO/Tanda&nbsp;Tangan</b></th>
						...
						<tbody>
							...
							// ttd nama
							$param = array();
							$param['id_user'] = $row->ttd_id_user;
							$ttd_nama = $sdm->getData('nama_karyawan_by_id',$param);
							...
							// pelaksana
							$pelaksana_ui = '';
							$sql2 = "select d.nama, p.manhour from sdm_user_detail d, wo_pengembangan_pelaksana p where p.id_wo_pengembangan='".$row->id."' and p.id_user=d.id_user order by d.nama ";
							...
							?>
							<tr>
								<td class="align-top" rowspan="3"><?=$i?>.</td>
								<td class="align-top" rowspan="2"><?=$row->id?></td>
								<td class="align-top"><?=$row->nama?></td>
								<td class="align-top" rowspan="2"><?=$row->no_wo.'<br/>'.$row->nama_wo?></td>
								<td class="align-top" rowspan="2"><?=$row->tahun.'<br/>'.$row->kategori.'<br/>'.$row->kategori2?></td>
								<td class="align-top" rowspan="2"><?=$tanggal?></td>
								<td class="align-top" rowspan="2"><?=$status_lock?></td>
								<td class="align-top" rowspan="2">
									<? if($sdm->isSA() || $row->id_pemberi_tugas==$_SESSION['sess_admin']['id']) { ?>
									<div class="input-group">
										<button class="btn btn-outline-primary dropdown-toggle" type="button" data-toggle="dropdown">Aksi</button>
										<div class="dropdown-menu dropdown-menu-right text-right">
											<a class="dropdown-item" href="<?=BE_MAIN_HOST?>/manpro/proyek/wo-pengembangan-update?id=<?=$row->id?>"><i class="os-icon os-icon-edit-1"> Update Data</i></a>
											<a class="dropdown-item" href="<?=BE_MAIN_HOST?>/manpro/proyek/wo-pengembangan-kunci?id=<?=$row->id?>"><i class="os-icon os-icon-alert-octagon"> Update Status Data</i></a>
