...
class Lembur extends db
{
	...
	function rekapManhour($tahun, $bulan, $is_cron, $bikin_session_info)
	{
		...
		if (strlen(mysqli_error($this->con)) > 0) {
			$sqlX2 .= mysqli_error($this->con) . "; ";
			$ok = false;
		}
		$sqlX1 .= $sql . "; ";
				...
				while ($row2 = mysqli_fetch_object($res2)) {
					...
					$ui_detail .=
						'<tr>
							...
							<td style="align:left;vertical-align:top;mso-number-format:\@">' . number_format($row2->jam_lembur, 1, '.', '') . '</td>
