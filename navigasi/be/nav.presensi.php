...
if($this->pageLevel2==""){
	...
	if($this->pageLevel3=="masuk") {
		...
		// bulan - tahun
		if(empty($tgl_mulai)) { $tgl_mulai = date("01-m-Y"); }
	...
	if($this->pageLevel3=="") {
		...
		if(!empty($kategori)) {
			if($kategori=="tepat_waktu") { $addSql .= " and (p.tipe='hadir' or p.tipe='hadir_khusus') and p.detik_terlambat=0 "; }
			else if($kategori=="terlambat") { $addSql .= " and (p.tipe='hadir' or p.tipe='hadir_khusus') and p.detik_terlambat>0 "; }
			else if($kategori=="tugas_luar") { $addSql .= " and (p.posisi='tugas_luar') "; }
			else { $addSql .= " and p.tipe='".$kategori."' "; }
		}
		if(!empty($kesehatan)) {
...
*/
	...
	if($this->pageLevel3=="") {
		...
		while($row=mysqli_fetch_object($res)) {
			...
			switch($row->shift) {
				case "1" : $info = "Shift Pagi"; break;
				case "2" : $info = "Shift Siang"; break;
				case "3" : $info = "Shift Malam"; break;
				default: break;
			}
	...
	if($this->pageLevel3=="konfig-jam-shift") {
		...
		if($_POST) {
			...
			if(empty($day_shift1_masuk)) { $strError .= '<li>Jam Shift 1 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift1_masuk)) $strError .= "<li>Format Jam Shift 1 salah.</li>"; }
			if(empty($day_shift2_masuk)) { $strError .= '<li>Jam Shift 2 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift2_masuk)) $strError .= "<li>Format Jam Shift 2 salah.</li>"; }
			if(empty($day_shift3_masuk)) { $strError .= '<li>Jam Shift 3 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift3_masuk)) $strError .= "<li>Format Jam Shift 3 salah.</li>"; }
			if(empty($day_shift_durasi)) $strError .= "<li>Durasi shift masih kosong.</li>";
			
			if(empty($day_shift1_masuk_listrik)) { $strError .= '<li>Jam masuk listrik shift 1 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift1_masuk_listrik)) $strError .= "<li>Format jam masuk listrik shift 1 salah.</li>"; }
			if(empty($day_shift1_pulang_listrik)) { $strError .= '<li>Jam pulang listrik shift 1 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift1_pulang_listrik)) $strError .= "<li>Format jam pulang listrik shift 1 salah.</li>"; }
			if(empty($day_shift2_masuk_listrik)) { $strError .= '<li>Jam masuk listrik shift 2 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift2_masuk_listrik)) $strError .= "<li>Format jam masuk listrik shift 2 salah.</li>"; }
			if(empty($day_shift2_pulang_listrik)) { $strError .= '<li>Jam pulang listrik shift 2 masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift2_pulang_listrik)) $strError .= "<li>Format jam pulang listrik shift 2 salah.</li>"; }
			
			if(empty($day_shift_masuk_min)) { $strError .= '<li>Jam Minimal Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_shift_masuk_min)) $strError .= "<li>Jam Minimal Presensi Masuk salah.</li>"; }
			...
			if(strlen($strError)<=0) {
				...
				if(strlen(mysqli_error($presensi->con))>0) { $sqlX2 .= mysqli_error($presensi->con)."; "; $ok = false; } $sqlX1 .= $sql."; ";
		...
		if($_POST) {
			...
			if(empty($medan_day_shift1_masuk)) { $strError .= '<li>Jam Shift 1 masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_shift1_masuk)) $strError .= "<li>Format Jam Shift 1 salah.</li>"; }
			if(empty($medan_day_shift2_masuk)) { $strError .= '<li>Jam Shift 2 masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_shift2_masuk)) $strError .= "<li>Format Jam Shift 2 salah.</li>"; }
			if(empty($medan_day_shift3_masuk)) { $strError .= '<li>Jam Shift 3 masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_shift3_masuk)) $strError .= "<li>Format Jam Shift 3 salah.</li>"; }
			if(empty($medan_day_shift_durasi)) $strError .= "<li>Durasi shift masih kosong.</li>";
			
			if(empty($medan_day_shift_masuk_min)) { $strError .= '<li>Jam Minimal Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_shift_masuk_min)) $strError .= "<li>Jam Minimal Presensi Masuk salah.</li>"; }
		...
		if($_POST) {
			...
			if(empty($day_monday_masuk)) { $strError .= '<li>Jam Masuk Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_monday_masuk)) $strError .= "<li>Format Jam Masuk Senin salah.</li>"; }
			if(empty($day_monday_pulang)) { $strError .= '<li>Jam Pulang Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_monday_pulang)) $strError .= "<li>Format Jam Pulang Senin salah.</li>"; }
			if(empty($day_tuesday_masuk)) { $strError .= '<li>Jam Masuk Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_tuesday_masuk)) $strError .= "<li>Format Jam Masuk Selasa salah.</li>"; }
			if(empty($day_tuesday_pulang)) { $strError .= '<li>Jam Pulang Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_tuesday_pulang)) $strError .= "<li>Format Jam Pulang Selasa salah.</li>"; }
			if(empty($day_wednesday_masuk)) { $strError .= '<li>Jam Masuk Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_wednesday_masuk)) $strError .= "<li>Format Jam Masuk Rabu salah.</li>"; }
			if(empty($day_wednesday_pulang)) { $strError .= '<li>Jam Pulang Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_wednesday_pulang)) $strError .= "<li>Format Jam Pulang Rabu salah.</li>"; }
			if(empty($day_thursday_masuk)) { $strError .= '<li>Jam Masuk Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_thursday_masuk)) $strError .= "<li>Format Jam Masuk Kamis salah.</li>"; }
			if(empty($day_thursday_pulang)) { $strError .= '<li>Jam Pulang Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_thursday_pulang)) $strError .= "<li>Format Jam Pulang Kamis salah.</li>"; }
			if(empty($day_friday_masuk)) { $strError .= '<li>Jam Masuk Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_friday_masuk)) $strError .= "<li>Format Jam Masuk Jumat salah.</li>"; }
			if(empty($day_friday_pulang)) { $strError .= '<li>Jam Pulang Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_friday_pulang)) $strError .= "<li>Format Jam Pulang Jumat salah.</li>"; }
			
			if(empty($day_saturday_masuk)) { $strError .= '<li>Jam Masuk Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_saturday_masuk)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($day_saturday_pulang)) { $strError .= '<li>Jam Pulang Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_saturday_pulang)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($day_sunday_masuk)) { $strError .= '<li>Jam Masuk Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_sunday_masuk)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			if(empty($day_sunday_pulang)) { $strError .= '<li>Jam Pulang Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_sunday_pulang)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			
			if(empty($day_reguler_masuk_min)) { $strError .= '<li>Batas Awal Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_reguler_masuk_min)) $strError .= "<li>Format Jam Batas Awal Presensi Masuk salah.</li>"; }
			if(empty($day_reguler_masuk_max)) { $strError .= '<li>Jam Batas Akhir Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_reguler_masuk_max)) $strError .= "<li>Format Jam Batas Akhir Presensi Masuk salah.</li>"; }
			if(empty($day_reguler_max_pulang)) { $strError .= '<li>Jam Batas Akhir Presensi Pulang masih kosong.</li>'; }
			else { if(!$umum->validateTime($day_reguler_max_pulang)) $strError .= "<li>Format Jam Batas Akhir Presensi Pulang salah.</li>"; }
		...
		if($_POST) {
			...
			if(empty($medan_day_monday_masuk)) { $strError .= '<li>Jam Masuk Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_monday_masuk)) $strError .= "<li>Format Jam Masuk Senin salah.</li>"; }
			if(empty($medan_day_monday_pulang)) { $strError .= '<li>Jam Pulang Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_monday_pulang)) $strError .= "<li>Format Jam Pulang Senin salah.</li>"; }
			if(empty($medan_day_tuesday_masuk)) { $strError .= '<li>Jam Masuk Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_tuesday_masuk)) $strError .= "<li>Format Jam Masuk Selasa salah.</li>"; }
			if(empty($medan_day_tuesday_pulang)) { $strError .= '<li>Jam Pulang Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_tuesday_pulang)) $strError .= "<li>Format Jam Pulang Selasa salah.</li>"; }
			if(empty($medan_day_wednesday_masuk)) { $strError .= '<li>Jam Masuk Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_wednesday_masuk)) $strError .= "<li>Format Jam Masuk Rabu salah.</li>"; }
			if(empty($medan_day_wednesday_pulang)) { $strError .= '<li>Jam Pulang Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_wednesday_pulang)) $strError .= "<li>Format Jam Pulang Rabu salah.</li>"; }
			if(empty($medan_day_thursday_masuk)) { $strError .= '<li>Jam Masuk Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_thursday_masuk)) $strError .= "<li>Format Jam Masuk Kamis salah.</li>"; }
			if(empty($medan_day_thursday_pulang)) { $strError .= '<li>Jam Pulang Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_thursday_pulang)) $strError .= "<li>Format Jam Pulang Kamis salah.</li>"; }
			if(empty($medan_day_friday_masuk)) { $strError .= '<li>Jam Masuk Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_friday_masuk)) $strError .= "<li>Format Jam Masuk Jumat salah.</li>"; }
			if(empty($medan_day_friday_pulang)) { $strError .= '<li>Jam Pulang Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_friday_pulang)) $strError .= "<li>Format Jam Pulang Jumat salah.</li>"; }
			
			if(empty($medan_day_saturday_masuk)) { $strError .= '<li>Jam Masuk Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_saturday_masuk)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($medan_day_saturday_pulang)) { $strError .= '<li>Jam Pulang Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_saturday_pulang)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($medan_day_sunday_masuk)) { $strError .= '<li>Jam Masuk Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_sunday_masuk)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			if(empty($medan_day_sunday_pulang)) { $strError .= '<li>Jam Pulang Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_sunday_pulang)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			
			if(empty($medan_day_reguler_masuk_min)) { $strError .= '<li>Batas Awal Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_reguler_masuk_min)) $strError .= "<li>Format Jam Batas Awal Presensi Masuk salah.</li>"; }
			if(empty($medan_day_reguler_masuk_max)) { $strError .= '<li>Jam Batas Akhir Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_reguler_masuk_max)) $strError .= "<li>Format Jam Batas Akhir Presensi Masuk salah.</li>"; }
			if(empty($medan_day_reguler_max_pulang)) { $strError .= '<li>Jam Batas Akhir Presensi Pulang masih kosong.</li>'; }
			else { if(!$umum->validateTime($medan_day_reguler_max_pulang)) $strError .= "<li>Format Jam Batas Akhir Presensi Pulang salah.</li>"; }
		...
		if($_POST) {
			...
			if(empty($poliklinik_day_monday_masuk)) { $strError .= '<li>Jam Masuk Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_monday_masuk)) $strError .= "<li>Format Jam Masuk Senin salah.</li>"; }
			if(empty($poliklinik_day_monday_pulang)) { $strError .= '<li>Jam Pulang Senin masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_monday_pulang)) $strError .= "<li>Format Jam Pulang Senin salah.</li>"; }
			if(empty($poliklinik_day_tuesday_masuk)) { $strError .= '<li>Jam Masuk Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_tuesday_masuk)) $strError .= "<li>Format Jam Masuk Selasa salah.</li>"; }
			if(empty($poliklinik_day_tuesday_pulang)) { $strError .= '<li>Jam Pulang Selasa masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_tuesday_pulang)) $strError .= "<li>Format Jam Pulang Selasa salah.</li>"; }
			if(empty($poliklinik_day_wednesday_masuk)) { $strError .= '<li>Jam Masuk Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_wednesday_masuk)) $strError .= "<li>Format Jam Masuk Rabu salah.</li>"; }
			if(empty($poliklinik_day_wednesday_pulang)) { $strError .= '<li>Jam Pulang Rabu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_wednesday_pulang)) $strError .= "<li>Format Jam Pulang Rabu salah.</li>"; }
			if(empty($poliklinik_day_thursday_masuk)) { $strError .= '<li>Jam Masuk Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_thursday_masuk)) $strError .= "<li>Format Jam Masuk Kamis salah.</li>"; }
			if(empty($poliklinik_day_thursday_pulang)) { $strError .= '<li>Jam Pulang Kamis masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_thursday_pulang)) $strError .= "<li>Format Jam Pulang Kamis salah.</li>"; }
			if(empty($poliklinik_day_friday_masuk)) { $strError .= '<li>Jam Masuk Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_friday_masuk)) $strError .= "<li>Format Jam Masuk Jumat salah.</li>"; }
			if(empty($poliklinik_day_friday_pulang)) { $strError .= '<li>Jam Pulang Jumat masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_friday_pulang)) $strError .= "<li>Format Jam Pulang Jumat salah.</li>"; }
			
			if(empty($poliklinik_day_saturday_masuk)) { $strError .= '<li>Jam Masuk Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_saturday_masuk)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($poliklinik_day_saturday_pulang)) { $strError .= '<li>Jam Pulang Sabtu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_saturday_pulang)) $strError .= "<li>Format Jam Pulang Sabtu salah.</li>"; }
			if(empty($poliklinik_day_sunday_masuk)) { $strError .= '<li>Jam Masuk Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_sunday_masuk)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			if(empty($poliklinik_day_sunday_pulang)) { $strError .= '<li>Jam Pulang Minggu masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_sunday_pulang)) $strError .= "<li>Format Jam Pulang Minggu salah.</li>"; }
			
			if(empty($poliklinik_day_reguler_masuk_min)) { $strError .= '<li>Batas Awal Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_reguler_masuk_min)) $strError .= "<li>Format Jam Batas Awal Presensi Masuk salah.</li>"; }
			if(empty($poliklinik_day_reguler_masuk_max)) { $strError .= '<li>Jam Batas Akhir Presensi Masuk masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_reguler_masuk_max)) $strError .= "<li>Format Jam Batas Akhir Presensi Masuk salah.</li>"; }
			if(empty($poliklinik_day_reguler_max_pulang)) { $strError .= '<li>Jam Batas Akhir Presensi Pulang masih kosong.</li>'; }
			else { if(!$umum->validateTime($poliklinik_day_reguler_max_pulang)) $strError .= "<li>Format Jam Batas Akhir Presensi Pulang salah.</li>"; }
		...
		if($_POST) {
			...
			if($gps_kantor_pusat_is_enabled && empty($gps_kantor_pusat_radius)) { $strError .= '<li>Radius kantor pusat masih kosong.</li>'; }
			if($gps_kantor_jogja_is_enabled && empty($gps_kantor_jogja_radius)) { $strError .= '<li>Radius kantor jogja masih kosong.</li>'; }
			if($gps_kantor_medan_is_enabled && empty($gps_kantor_medan_radius)) { $strError .= '<li>Radius kantor medan masih kosong.</li>'; }
			if($gps_poliklinik_is_enabled && empty($gps_poliklinik_radius)) { $strError .= '<li>Radius poliklinik masih kosong.</li>'; }
