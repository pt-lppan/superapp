<?
	...
	$this->pageTitle = "Riwayat Organisasi Non Formal";
	...
	if($_POST) {
		...
		if(strlen($strError)<=0) {
			...
			if($ok==true) {
				...
				$manpro->insertLog('berhasil update data riwayat Organisasi Non Formal ('.$id.')','',$sqlX2);
				$_SESSION['result_info'] = "Data berhasil disimpan.";
				header("location:?m=".$m."&id=".$id);exit;
			} else {
				mysqli_query($manpro->con, "ROLLBACK");
				$manpro->insertLog('gagal update data  riwayat Organisasi Non Formal  ('.$id.')','',$sqlX2);
