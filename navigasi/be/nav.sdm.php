...
if ($this->pageLevel2 == "") {
	...
	if ($this->pageLevel3 == "covid") {
		...
		while ($row = mysqli_fetch_object($res)) {
			...
			if ($dif < 0) {
				$juml++;
				$biodata = $row->last_update_pribadi;
			}
			...
			if ($dif < 0) {
				$juml++;
				$pendidikan = $row->last_update_didik;
			}
			...
			if ($dif < 0) {
				$juml++;
				$pelatihan = $row->last_update_latih;
			}
			...
			if ($dif < 0) {
				$juml++;
				$jabatan = $row->last_update_jabatan;
			}
			...
			if ($dif < 0) {
				$juml++;
				$prestasi = $row->last_update_prestasi;
			}
			...
			if ($dif < 0) {
				$juml++;
				$nilai_visi = $row->last_update_nilai_interest;
			}
			...
			if ($dif < 0) {
				$juml++;
				$penugasan = $row->last_update_penugasan;
			}
			...
			if ($dif < 0) {
				$juml++;
				$org_profesional = $row->last_update_org_profesi;
			}
			...
			if ($dif < 0) {
				$juml++;
				$org_non_formal = $row->last_update_org_non_formal;
			}
			...
			if ($dif < 0) {
				$juml++;
				$publikasi = $row->last_update_publikasi;
			}
			...
			if ($dif < 0) {
				$juml++;
				$pembicara = $row->last_update_pembicara;
			}
		...
		$addSql = '';
		if (!empty($fstat)) {
			$addSql .= " and d.status_karyawan='" . $fstat . "' ";
		}
		...
		if (!empty($tahun_pensiun)) {
			$addSql .= " and ud.tgl_pensiun like '" . $tahun_pensiun . "%' ";
		}
		if (!empty($statK)) {
			$addSql .= " and ud.status_karyawan='" . $statK . "' ";
		}
		if (!empty($statD)) {
			$addSql .= " and u.status='" . $statD . "' ";
		}
		...
		$sql =
			"SELECT
				count(m.id_user) as juml, ud.id_user, ud.posisi_presensi, ud.jk, ud.telp, ud.ktp, ud.alamat, ud.tgl_bebas_tugas, ud.tgl_pensiun, ud.npwp, ud.bpjs_kesehatan, ud.bpjs_ketenagakerjaan, ud.email, ud.level_karyawan, ud.inisial, ud.nama, ud.nik, ud.tgl_lahir, ud.tgl_masuk_kerja, ud.tgl_pengangkatan, ud.tgl_berakhir_kontrak, ud.tgl_rotasi_cuti, ud.bln_rotasi_cuti, ud.tgl_bebas_tugas, ud.tgl_pensiun, ud.nama_pasangan, 
		...
		while ($row = mysqli_fetch_object($res)) {
			...
			$ui .=
				'<tr>
					...
					<td class="align-top">' . $row->tgl_berakhir_kontrak . '</td>
				 </tr>';
		}
	...
	if ($_POST) {
		...
		if (empty($pass_l)) {
			...
			$res = mysqli_query($sdm->con, $sql);
			$row = mysqli_fetch_object($res);
			if (!$sdm->validatePassword($pass_l, $row->hash, $row->password)) {
				$strError .= '<li>Password lama salah.</li>';
			}
		}
		if (empty($pass_b)) {
			$strError .= '<li>Password baru masih kosong.</li>';
		} else {
			if (strlen($pass_b) < PASSWORD_MIN_CHARS) {
				$strError .= '<li>Password baru minimal ' . PASSWORD_MIN_CHARS . ' karakter.</li>';
			}
		}
		if (empty($pass_c)) {
			$strError .= '<li>Konfirmasi password baru masih kosong.</li>';
		} else {
			if (strlen($pass_c) < PASSWORD_MIN_CHARS) {
				$strError .= '<li>Konfirmasi password baru minimal ' . PASSWORD_MIN_CHARS . ' karakter.</li>';
			}
		}
		if ((!empty($pass_b) && !empty($pass_c)) && $pass_b != $pass_c) {
			$strError .= '<li>Password baru dan konfirmasi password baru tidak sama.</li>';
		}
		if ($pass_l === $pass_b && !empty($pass_b)) {
	...
	if ($this->pageLevel3 == "") {
		...
		if (strlen($strError) <= 0) {
			header("location:" . BE_MAIN_HOST . "/sdm/struktur/" . $kat_data . "?kat_sk=" . $kat_sk);
			exit;
		}
		...
		if ($_POST) {
			...
			if (strlen($strError) <= 0) {
				...
				if ($ok == true) {
					...
					header("location:" . BE_MAIN_HOST . "/sdm/struktur/unitkerja?kat_sk=" . $kat_sk);
					exit;
				} else {
		...
		if ($_POST) {
			...
			if (strlen($strError) <= 0) {
				...
				if ($ok == true) {
					...
					$_SESSION['result_info'] = "Data berhasil disimpan.";
					header("location:" . BE_MAIN_HOST . "/sdm/struktur/jabatan?kat_sk=" . $kat_sk);
					exit;
				} else {
					mysqli_query($sdm->con, "ROLLBACK");
		...
		if ($_POST) {
			...
			if (strlen($strError) <= 0) {
				...
				if ($ok == true) {
					...
					$_SESSION['result_info'] = "Data berhasil disimpan.";
					header("location:" . BE_MAIN_HOST . "/sdm/struktur/karyawan");
					exit;
				} else {
					mysqli_query($sdm->con, "ROLLBACK");
	...
	if ($this->pageLevel3 == "update") {
		...
		if ($_do == "update_status") {
			...
			$_SESSION['result_info'] = "Data berhasil disimpan.";
			header("location:" . BE_MAIN_HOST . "/sdm/unit-kerja");
			exit;
		}
	...
	if ($this->pageLevel3 == "update") {
		...
		if ($_do == "update_status") {
			...
			$_SESSION['result_info'] = "Data berhasil disimpan.";
			header("location:" . BE_MAIN_HOST . "/sdm/jabatan");
			exit;
		}
		if ($_do == "update_status_read") {
	...
	if ($_POST) {
		...
		if (strlen($strError) <= 0) {
			...
			if ($ok == true) {
				...
				$_SESSION['result_info'] = "Data berhasil disimpan.";
				header("location:" . BE_MAIN_HOST . "/sdm/konfigurasi_update_data_karyawan");
				exit;
			} else {
				mysqli_query($sdm->con, "ROLLBACK");
	...
	if ($this->pageLevel3 == "") {
		...
		$addSql = "";
		if (!empty($inisial)) {
			$addSql .= " and (d.inisial like '%" . $inisial . "%') ";
		}
		if (!empty($nik)) {
			$addSql .= " and (d.nik like '%" . $nik . "%') ";
		}
		if (!empty($nama)) {
			$addSql .= " and (d.nama like '%" . $nama . "%') ";
		}
		if (!empty($posisi_presensi)) {
			$addSql .= " and (d.posisi_presensi='" . $posisi_presensi . "') ";
		}
		...
		if ($id < 1) {
			...
			$tgl_berakhir_kontrak = $umum->tglDB2Indo($row->tgl_berakhir_kontrak, "dmY");
		...
		if ($_POST) {
			...
			$tgl_berakhir_kontrak = $security->teksEncode($_POST['tgl_berakhir_kontrak']);
			...
			$tgl_berakhir_kontrakDB = $umum->tglJamIndo2DB($tgl_berakhir_kontrak);
			...
			if (strlen($strError) <= 0) {
				...
				if ($mode == "add") {
					...
					$sql =
						"insert into sdm_user_detail set
							...
							tgl_berakhir_kontrak='" . $tgl_berakhir_kontrakDB . "',
				...
				if ($ok == true) {
					...
					$_SESSION['result_info'] = "Data berhasil disimpan.";
					header("location:" . BE_MAIN_HOST . "/sdm/karyawan/update?id=" . $id);
					exit;
				} else {
					mysqli_query($sdm->con, "ROLLBACK");
...
*/ else if ($this->pageLevel2 == "ajax") { // ajax
	...
	if ($act == "karyawan") {
		...
		echo $html;
	} else if ($act == "generate_nik") {
		header('Content-Type: application/json');

		// 1. Ambil dan validasi data dari request
		$jenis_karyawan = $security->teksEncode($_GET['jenis_karyawan']);
		$tanggal_input = $security->teksEncode($_GET['tanggal']); // Format: dd-mm-yyyy

		if (empty($jenis_karyawan) || empty($tanggal_input)) {
			echo json_encode(['error' => 'Data tidak lengkap.']);
			exit;
		}

		// 2. Tentukan Prefix dan Kolom Tanggal berdasarkan Jenis Karyawan
		$prefix = '';
		$kolom_tanggal_db = '';
		$prefix_tetap = 'TTP'; // Asumsi prefix untuk Karyawan Tetap, bisa diubah

		if ($jenis_karyawan === 'kontrak') {
			$prefix = 'KTR';
			$kolom_tanggal_db = 'tgl_masuk_kerja';
		} else if ($jenis_karyawan === 'tetap') {
			$prefix = $prefix_tetap;
			$kolom_tanggal_db = 'tgl_pengangkatan';
		} else {
			echo json_encode(['error' => 'Jenis Karyawan tidak valid.']);
			exit;
		}

		// 3. Proses tanggal untuk mendapatkan Tahun dan Bulan
		// Ubah format dd-mm-yyyy menjadi objek DateTime
		$date = DateTime::createFromFormat('d-m-Y', $tanggal_input);
		if (!$date) {
		...
		// 4. Hitung urutan karyawan berdasarkan jenis, tahun, dan bulan yang sama
		$sql = "SELECT COUNT(*) as jumlah 
            FROM sdm_user_detail 
            WHERE jenis_karyawan = ? 
              AND YEAR(" . $kolom_tanggal_db . ") = ? 
              AND MONTH(" . $kolom_tanggal_db . ") = ?";

		// Gunakan prepared statement untuk keamanan
		$stmt = mysqli_prepare($sdm->con, $sql);
		mysqli_stmt_bind_param($stmt, "sss", $jenis_karyawan, $tahun, $bulan);
		...
		// 5. Buat NIK baru dengan format yang diinginkan
		// Contoh: KTR + 2025 + 10 + 002 (jika urutan ke-2 dan padding 3 digit)
		// Sesuaikan '3' dengan panjang nomor urut yang Anda inginkan
		$nomor_urut_terformat = $urutan;

		$nik_baru = $prefix . $tahun . $bulan . $nomor_urut_terformat;

		// 6. Kirim NIK baru sebagai JSON
		echo json_encode(['nik' => $nik_baru]);
		exit;
	}
	exit;
	...
	if ($act == "update_status") {
		...
		if ($id_user < 1) $strError .= "Karyawan masih kosong.\n";
		if (empty($password)) {
			$strError .= "Password masih kosong.\n";
		}
		if (!empty($password) && strlen($password) < PASSWORD_MIN_CHARS) {
			$strError .= "Password minimal " . PASSWORD_MIN_CHARS . " karakter.\n";
		}
