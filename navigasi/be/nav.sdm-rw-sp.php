<?
	...
	$this->pageTitle = "Riwayat Surat Peringatan";
	$this->pageName = "rw-sp";
	...
	$arrK = $umum->getKategori('kategori_sp');
	
	$qD='select last_update_sp,status_karyawan,nik,nama from sdm_user_detail where id="'.$id.'"';
	$data1 = $manpro->doQuery($qD,0,'object');
	$namakaryawan=$data1[0]->nama;
	$nik=$data1[0]->nik;
	$last_update=$data1[0]->last_update_sp;
	$_stt=$umum->getKategori("status_karyawan");
	$status_karyawan=$_stt[$data1[0]->status_karyawan];
	
	$strError = "";
	$prefix_url = MEDIA_HOST."/sdm/sp";
	$prefix_folder = MEDIA_PATH."/sdm/sp";
	$prefix_berkas = $nik;
	...
	// internal
	$sql =
		"select * from  sdm_history_teguran
	...
	foreach($data2 as $row) {
		...
		$addJS2 .= 'setupDetail("'.$i.'",1,"'.$row->id.'","'.$umum->reformatText4Js($row->no_surat).'","'.$umum->reformatText4Js($row->tanggal).'","'.$umum->reformatText4Js($row->hal).'","'.$umum->reformatText4Js($row->kategori).'","'.$umum->reformatText4Js($berkas).'",1);';
	}
	$addJS2 .= 'num='.$i.';';
	...
	if($_POST) {
		...
		foreach($det as $key => $val) {
			...
			$no_surat = $security->teksEncode($val[1]);
			$tanggal = $security->teksEncode($val[2]);
			$hal = $security->teksEncode($val[3]);
			$kategori = $security->teksEncode($val[4]);
			...
			if(empty($no_surat)) $strError .= "<li>No SK pada baris ke ".$key." masih kosong.</li>";
			if(empty($tanggal)) $strError .= "<li>Tanggal pada baris ke ".$key." masih kosong.</li>";
			if(empty($hal)) $strError .= "<li>Perihal pada baris ke ".$key." masih kosong.</li>";
			...
			$addJS2 .= 'setupDetail("'.$i.'",1,"'.$val[0].'","'.$umum->reformatText4Js($val[1]).'","'.$umum->reformatText4Js($val[2]).'","'.$umum->reformatText4Js($val[3]).'","'.$umum->reformatText4Js($val[4]).'","'.$umum->reformatText4Js($berkas).'",1);';
		}
		$addJS2 .= 'num='.$i.';';
		...
		if(strlen($strError)<=0) {
			...
			$arr = array();
			$arrB = array();
			$sql = "select id, berkas from sdm_history_teguran where id_user='".$id."' and status='1' ";
			...
			foreach($det as $key => $val) {
				...
				if($did>0) { // update datanya
					$sql = "update sdm_history_teguran set no_surat='".$no_surat."', 
					tanggal='".$tanggal."', 
					hal='".$hal."', 
					kategori='".$kategori."'
					where id='".$did."'";
					...
					if(is_uploaded_file($_FILES['berkas_'.$key]['tmp_name'])){
						...
						$sql4 = "update sdm_history_teguran set berkas='".$namafile."' where id='".$did."'";
						//echo $sql4.'---> upload';
					...
					$sql = "insert into sdm_history_teguran set  
					id_user='".$id."',
					no_surat='".$no_surat."', 
					hal='".$hal."', 
					tanggal='".$tanggal."', 
					...
					if(is_uploaded_file($_FILES['berkas_'.$key]['tmp_name'])){
						...
						$sql2x = "update sdm_history_teguran set berkas='".$namafile."' where id='".$new_id."'";
						//echo $sql2x;die();
				...
				$sql2=' update sdm_user_detail set last_update_sp="'.date("Y-m-d H:i:s").'" where id="'.$id.'"';
			...
			foreach($arr as $key => $val) {
				$sql = "update sdm_history_teguran set status='0' where id='".$key."' ";
				$res = mysqli_query($manpro->con,$sql);
				
				/* biarkan file tetap di server
				$namafile=$nik.'_'.$key.'.pdf';
				$namafileexits=$prefix_folder.'/'.$folder.'/'.$namafile;				
			...
			if($ok==true) {
				...
				$manpro->insertLog('berhasil update data riwayat Teguran ('.$id.')','',$sqlX2);
				$_SESSION['result_info'] = "Data berhasil disimpan.";
				header("location:?m=".$m."&id=".$id);exit;
			} else {
				mysqli_query($manpro->con, "ROLLBACK");
				$manpro->insertLog('gagal update data  riwayat Teguran  ('.$id.')','',$sqlX2);
