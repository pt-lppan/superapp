...
class func
{
	...
	function setupPaginationUI($query, $connection, $limit = 10, $current_page, $targetpage = "/", $pagestring = "?page=", $pos = "L", $enableAll = false, $disableBar = false, $query_count = "")
	{
		...
		$pos = strtoupper($pos);
		if ($pos == "L") {
			$posStyle = ' justify-content-start" ';
		} else if ($pos == "R") {
			$posStyle = ' justify-content-end ';
		} else if ($pos == "C") {
			$posStyle = ' justify-content-center" ';
		}
	...
	function checkboxUI($arr, $kat, $tagName, $class = "", $seld = "")
	{
		...
		foreach ($arr as $key => $value) {
			if (is_array($arrSeld)) {
				$seldItem = (in_array($key, $arrSeld)) ? 'checked="checked"' : '';
			}
	...
	function strtotime_bugfix($tanggal, $alterThn = 0, $alterBln = 0, $alterTgl = 0, $alterJam = 0, $alterMnt = 0, $alterDtk = 0)
	{
		...
		$alterThn = (int) $alterThn;
		$tgl['0'] += $alterThn;
		$alterBln = (int) $alterBln;
		$tgl['1'] += $alterBln;
		$alterTgl = (int) $alterTgl;
		$tgl['2'] += $alterTgl;
		$alterJam = (int) $alterJam;
		$jam['0'] += $alterJam;
		$alterMnt = (int) $alterMnt;
		$jam['1'] += $alterMnt;
		$alterDtk = (int) $alterDtk;
		$jam['2'] += $alterDtk;
	...
	function detik2jam($detik, $format = 'hm')
	{
		...
		if ($format == "h") {
			...
			$hasil = sprintf('%02d:%02d', $h, $m);
		} else if ($format == "hms") {
			$hasil = sprintf('%02d:%02d:%02d', $h, $m, $s);
